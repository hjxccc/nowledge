...
### 2.3 学习价值打分维度（换掉 last30days 的新闻维度）

last30days 按 freshness+热度选新闻；nowledge 换成**对学这个主题最有用**：
...
**模型/代码分工（抄卡兹克 11 版血泪）**：模型只做"给候选打分 + 写一句'为什么选它'"；选源、去重、聚类、角色配齐、排序全用**确定性代码**。防止"什么都交给模型，模型就什么都干不好"。
